...
import { signCardPqc } from './trust.js';
...
export interface A2AServerDeps {
  ...
  /**
   * Signs our published card with ML-DSA-65 alongside serving it. The entry
   * lives in the card's own `signatures` array, so verifiers that understand
   * only classical JWS skip it and PQC-aware ones verify it — nothing about
   * the card's shape changes either way.
   */
  pqcSigner?: { kid: string; secretKey: Uint8Array };
}
...
export async function buildAgentCard(deps: A2AServerDeps): Promise<AgentCard> {
  ...
  // PQC-sign when a signing key is configured: PQC-aware verifiers (our own
  // verifyCard) check the entry, classical-only verifiers skip the unknown
  // alg — nothing about the card's shape changes either way.
  if (deps.pqcSigner !== undefined) {
    const entry = signCardPqc(card, deps.pqcSigner.kid, deps.pqcSigner.secretKey);
    (card.signatures as unknown[]).push(entry);
  }
