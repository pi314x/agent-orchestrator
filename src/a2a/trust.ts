import { canonicalizeAgentCard, verifyAgentCardSignature, type AgentCard } from '@a2a-js/sdk';
import type { JWK } from 'jose';
import { OrchestratorError } from '../errors.js';
import { keyFingerprint, pqcSign, pqcVerify, PQC_SIGN_ALG } from '../core/pqc.js';
...
export async function verifyCard(
  ...
  pqcResolver: PqcKeyResolver = createPqcResolver()
): Promise<{ trustLevel: TrustLevel; reason?: string; alg?: string }> {
  ...
  try {
    ...
    // Classical verification failed — fall through to the PQC entries below
    // before calling the card unverified. The classical error is kept for the
    // final reason, so a card with neither kind of valid signature still says
    // why in the language its signer would recognise.
    const classicalReason = error instanceof Error ? error.message : 'Signature verification failed.';
    if (await verifyCardPqc(card, pqcResolver)) {
      return { trustLevel: 'verified', alg: PQC_SIGN_ALG };
    }
    return { trustLevel: 'unverified', reason: classicalReason };
  }
}

/**
 * Post-quantum card signatures (NIST FIPS 204 ML-DSA-65). Same envelope as
 * the classical path — JWS flattened JSON over the JCS-canonical card with
 * `signatures` excluded — with `alg: 'ML-DSA-65'` in the protected header, so
 * a verifier that understands only classical JWS skips the entry instead of
 * choking on it, and this verifier skips classical entries the same way.
 */
export type PqcKeyResolver = (kid: string, jku?: string) => Promise<Uint8Array>;

type PqcProtectedHeader = { alg?: unknown; kid?: unknown; jku?: unknown };

function decodePqcHeader(entry: { protected?: string }): PqcProtectedHeader | undefined {
  if (entry.protected === undefined || entry.protected === '') return undefined;
  try {
    return JSON.parse(Buffer.from(entry.protected, 'base64url').toString('utf8')) as PqcProtectedHeader;
  } catch {
    return undefined;
  }
}

export function signCardPqc(
  card: AgentCard,
  kid: string,
  secretKey: Uint8Array
): { protected: string; signature: string; header: undefined } {
  const payload = new TextEncoder().encode(canonicalizeAgentCard(card));
  const protectedHeader = Buffer.from(JSON.stringify({ alg: PQC_SIGN_ALG, kid, typ: 'PQC+JWS' })).toString(
    'base64url'
  );
  const signature = Buffer.from(pqcSign(secretKey, payload)).toString('base64url');
  return { protected: protectedHeader, signature, header: undefined };
}

/** True when any ML-DSA-65 entry on the card verifies. Never throws. */
export async function verifyCardPqc(card: AgentCard, resolver: PqcKeyResolver): Promise<boolean> {
  const payload = new TextEncoder().encode(canonicalizeAgentCard(card));
  for (const entry of card.signatures ?? []) {
    const header = decodePqcHeader(entry);
    if (header?.alg !== PQC_SIGN_ALG || typeof header.kid !== 'string') continue;
    const jku = typeof header.jku === 'string' ? header.jku : undefined;
    try {
      const publicKey = await resolver(header.kid, jku);
      if (publicKey.length !== 1952) continue;
      const signature = new Uint8Array(Buffer.from(entry.signature, 'base64url'));
      if (pqcVerify(publicKey, payload, signature)) return true;
    } catch {
      continue;
    }
  }
...
/**
 * Resolves an ML-DSA-65 public key the same way the classical resolver does:
 * the signature's `jku` names a JWKS document, fetched only after the usual
 * URL checks, holding entries shaped `{ "alg": "ML-DSA-65", "kid", "x" }`
 * with `x` the base64url 1952-byte public key. `kty` is deliberately not
 * matched — no registered JOSE `kty` names this key type yet.
 */
export function createPqcResolver(fetchImpl: typeof fetch = fetch): PqcKeyResolver {
  return async (kid, jku) => {
    if (jku === undefined || jku === '') {
      throw new Error('The PQC card signature carries no jku, so its key cannot be located.');
    }
    const url = validateFetchUrl(jku);
    const response = await fetchImpl(url.toString(), { headers: { accept: 'application/json' } });
    if (!response.ok) throw new Error(`JWKS fetch returned ${response.status}.`);

    const jwks = (await response.json()) as { keys?: { alg?: unknown; kid?: unknown; x?: unknown }[] };
    const key = (jwks.keys ?? []).find(candidate => candidate.kid === kid && candidate.alg === PQC_SIGN_ALG);
    if (key === undefined || typeof key.x !== 'string') {
      throw new Error(`JWKS at ${jku} has no ${PQC_SIGN_ALG} key with kid ${kid}.`);
    }
    const publicKey = new Uint8Array(Buffer.from(key.x, 'base64url'));
    if (publicKey.length !== 1952) throw new Error(`JWKS key ${kid} is not a 1952-byte ${PQC_SIGN_ALG} public key.`);
    return publicKey;
  };
}

/** Fingerprint for logs and tool output — identifies the key, never the secret. */
export function pqcKeyFingerprint(publicKey: Uint8Array): string {
