...
export const MIGRATIONS: readonly Migration[] = [
  ...
  },
  {
    ...
    name: 'grant_presets',
    up: `
      -- Named tool-grant bundles an admin curates once and anyone names in
      -- agent_create. Global like agent_templates (no owner column): saving
      -- is admin-gated, using is not. Grants are copied onto the agent at
      -- create time — no ongoing link, so renaming a server later cannot
      -- silently change what existing agents may call.
      CREATE TABLE grant_presets (
        name       TEXT PRIMARY KEY,
        spec       TEXT NOT NULL,
        created_at TEXT NOT NULL,
        updated_at TEXT NOT NULL
  ...
  },
  {
    ...
    name: 'schedules',
    up: `
      -- Cron-triggered work: each due row submits one job, then advances its
      -- own next_run_at in the same guarded statement, so two instances
      -- sharing a database never fire the same schedule twice. Names are
      -- unique per owner, like workflows. No catch-up runs: an overdue
      -- schedule fires once, immediately, and its next run counts from now.
  ...
  },
  {
    ...
    name: 'webhooks',
    up: `
      -- Completion callbacks: an admin registers a URL plus the settle
      -- events it wants, and settlement posts there. Owner-scoped like
      -- everything else — a webhook only ever hears about its own owner's
      -- jobs and runs, never another's.
  ...
  },
  {
    ...
    up: `
      -- Overlap policy per schedule: 'allow' fires even while the previous
      -- run is still going; 'skip' waits for it. last_job_id names the most
      -- recent firing so the claim can refuse a row whose job has not settled
      -- yet, in the same guarded statement that takes the row.
      ALTER TABLE schedules ADD COLUMN overlap TEXT NOT NULL DEFAULT 'allow';
  ...
  },
  {
    ...
    up: `
      -- Cron fields match wall-clock time in this IANA zone; NULL keeps the
      -- original UTC behaviour. Storage stays UTC instants either way.
      ALTER TABLE schedules ADD COLUMN timezone TEXT;
    `
  },
  {
    version: 18,
    name: 'grant_acceptance',
    up: `
      -- Shares need an accept step: a grant starts 'pending' and confers no
      -- visibility until the grantee accepts it, when it becomes 'accepted'.
      -- Existing rows predate acceptance and stay effective, so they land as
      -- 'accepted' — new shares insert 'pending' explicitly in code.
      ALTER TABLE resource_grants ADD COLUMN status TEXT NOT NULL DEFAULT 'accepted';
    `
  },
  {
    version: 19,
    name: 'agent_enabled',
    up: `
      -- Per-agent kill switch, toggled from the dashboard or agent_update.
      -- Existing rows predate the flag and were usable, so they land enabled —
      -- only agents synced from repo files afterwards start disabled, and the
      -- sync never rewrites the flag on update.
