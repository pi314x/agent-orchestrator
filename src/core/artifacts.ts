...
import { openText, sealText, type PqcDataKey } from './pqc.js';
 ...
 *
 * With a data key configured (`ORCH_PQC_DATA_KEY`, hybrid X25519+ML-KEM-768),
 * stored content is a sealed envelope and only the hash still names the
 * plaintext; without one everything stays plaintext. Mixed tables read back
 * either way, so turning encryption on needs no migration.
 */
export class ArtifactStore {
  ...
  async put(input: PutArtifactInput): Promise<ArtifactRecord> {
    // The hash names the logical content, so identical plaintext still shares
    // a fingerprint whether or not it is sealed at rest.
    const contentHash = createHash('sha256').update(input.content).digest('hex');
    const stored = this.dataKey === undefined ? input.content : sealText(this.dataKey, input.content);
    const sizeBytes = Buffer.byteLength(stored, 'utf8');
  ...
  async read(
    ...
    // Decrypt-then-slice: an offset into ciphertext addresses nothing, and a
    // sealed row read without a key fails here rather than leaking the box.
    const full = openText(this.dataKey, row.content ?? '');
  ...
  /**
   * Drop everything one job stored, so a retried attempt starts from a clean
   * slate instead of duplicating the failed attempt's artifacts. Internal:
   * only the scheduler calls this, for a job it owns the execution of.
   */
  async deleteForJob(jobId: string): Promise<number> {
    const result = await this.db.prepare('DELETE FROM artifacts WHERE job_id = ?').run(jobId);
