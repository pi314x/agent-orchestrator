...
import { assertTimeoutWithinLimit } from './policy.js';
import type { AgentLimits } from './registry.js';
...
/**
 * Both drivers surface unique violations differently — better-sqlite3 as
 * SQLITE_CONSTRAINT_UNIQUE, pg as 23505 — and both rethrow raw, so match on
...
export type AgentSnapshot = {
  ...
  toolGrants?: string[];
  /**
   * The agent's own caps, frozen at submit time like everything else here —
   * so a later agent_update cannot change a running job. Absent for rows
   * written before limits were snapshotted, which then run exactly as before.
   */
  limits?: AgentLimits;
...
export class JobStore {
  ...
  async create(input: CreateJobInput): Promise<JobRecord> {
    // The single choke point for every job row — tools, schedules, workflow
    // steps, declarative config, inbound A2A tasks. An unbounded timeout
    // would overflow the runner's setTimeout and fire after 1ms, so the bound
    // lives here and not in any one adapter.
    if (input.timeoutSec !== undefined) assertTimeoutWithinLimit(input.timeoutSec);
    ...
    try {
      ...
      // Lost a same-key race: a sibling submit passed its own find-then-check
      // before this insert landed, and the (owner_id, idempotency_key) index
      // rejected the duplicate. Hand back the winner's row — the whole point
      // of the key — instead of leaking a raw constraint error. Anything
      // without a key, or a key that still reads back nothing, is a genuine
      // failure and rethrows untouched.
      if (input.idempotencyKey === undefined || !isUniqueViolation(error)) throw error;
      const winner = await this.findByIdempotencyKey(input.idempotencyKey, ownerId);
  ...
  /**
   * The row for a (key, owner) pair — never another owner's. The unique index
   * is per owner, so an unscoped lookup would hand back someone else's job
   * (and its result text) to whoever merely guessed the key.
   */
  async findByIdempotencyKey(key: string, ownerId: string): Promise<JobRecord | undefined> {
    const row = (await this.db
      .prepare('SELECT * FROM jobs WHERE owner_id = ? AND idempotency_key = ?')
      .get(ownerId, key)) as JobRow | undefined;
    return row === undefined ? undefined : toRecord(row);
  }
  ...
  async countByState(state: JobState): Promise<number> {
    // Postgres returns COUNT(*) as a string, not a number: without this the
    // scheduler's `=== 0` check in drain() never fires there and
    // orchestrator_status fails its own output schema. Same shape as the
    // budget aggregate in BudgetTracker.spend.
  ...
  /**
   * The claim above, with the deployment caps folded into the same statement.
   * Reading the count first and claiming after leaves a window where two
   * instances both pass and both start — overshooting a `maxConcurrent`
   * budget by one per racer. The subqueries close that window: the UPDATE
   * only lands while the counted state still holds. Callers keep their
   * read-then-check fast paths and treat an `undefined` as either outcome;
   * the pre-checks just stop being load-bearing. COUNT(*) arrives as a
   * string on Postgres, but the comparison happens server-side as BIGINT,
   * so no normalization is needed here.
   */
  async claimWithCapacity(
    id: string,
    claimedBy: string | undefined,
    caps: { globalCap?: number; agentCap?: number; agentId?: string }
  ): Promise<JobRecord | undefined> {
    const now = new Date().toISOString();
    const conditions: string[] = ['id = ?', `state = 'queued'`];
    const params: unknown[] = [id];

    if (caps.globalCap !== undefined) {
      conditions.push(`(SELECT COUNT(*) FROM jobs WHERE state = 'running') < ?`);
      params.push(caps.globalCap);
    }
    if (caps.agentCap !== undefined && caps.agentId !== undefined) {
      conditions.push(`(SELECT COUNT(*) FROM jobs WHERE state = 'running' AND agent_id = ?) < ?`);
      params.push(caps.agentId, caps.agentCap);
    }
    ...
    const rows = (await this.db
      ...
      .all(now, now, claimedBy ?? null, now, ...params)) as JobRow[];
