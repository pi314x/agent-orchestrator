...
export const DEFAULT_ANTHROPIC_MODEL = 'sonnet-5';
...
/**
 * Cached list prices in USD per million tokens (2026-09). Only used to
...
function asOrchestratorError(error: unknown): OrchestratorError {
  ...
  // Retryable by the scheduler with backoff: rate limits, overloaded and
  // unreachable endpoints. Anything else — bad credentials, bad requests,
  // refusals — fails the job immediately and is never retried automatically.
  // (The SDK also retries inside its own calls; this sharpens only the outer,
  // scheduler-level retry with the server's own wait when it names one.)
  const retryAfterMs =
    error instanceof Anthropic.APIError && typeof error.headers?.get === 'function'
  ...
  if (
    ...
    return new OrchestratorError(
      'TRANSIENT',
      `Anthropic endpoint trouble (${error.constructor.name}): ${error.message}`,
...
export class AnthropicRunner implements Runner {
  ...
  private async *runAgentLoop(
    ...
    // assistant turn's prose is the result. Blockers and downstream context
    // ride along as `##` sections inside the same text.
