      ...
      'job_submit',
      {
        ...
        inputSchema: z.object({
          ...
          runner: z
            ...
            .optional()
            .describe(
              'Execution backend for this job, e.g. "cli" to spend your CLI login. Defaults to the deployment default. "sampling" is rejected here — a borrowed model only exists while a request is open, so use delegate.'
            ),
          context: z
        ...
        try {
          if (args.runner === 'sampling') {
            throw new OrchestratorError(
              'INVALID_INPUT',
              'Sampling executes inline within the calling request: use delegate instead.',
              'job_submit returns a handle for background work, but a borrowed model only exists while this request is open.'
            );
          }
          ...
          return toolOk({ job: toJobView(job) });
        } catch (error) {
    ...
    server.registerTool(
      ...
      async args => {
        try {
          ...
          return toolOk({ delivered: true, state: job.state });
        } catch (error) {
          return toolError(error);
        }
      }
