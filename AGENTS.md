...
- **The inbound A2A server is not wired up.** `src/a2a/server.ts` builds a card, an executor and a JSON-RPC handler, and nothing calls `createA2AServer` — only `buildAgentCard` is used, to compose the card on request. So `A2A_HTTP_PORT` listens on nothing, and three defects in `OrchestratorExecutor` are latent rather than live: an inbound job is submitted with no `timeoutSec` (it outlives the 55s task wait and keeps running after the task reports failure), `cancelTask` only records the id and never cancels the job, and the card is built once at construction so a withdrawn skill stays advertised. Fix those as part of wiring it up, with tests — do not "fix" them blind.
