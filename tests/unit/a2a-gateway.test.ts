...
describe('A2A gateway polling', () => {
  ...
  /** A remote agent that first asks a question, then finishes once answered. */
  function questioningClient(): {
    client: unknown;
    sent: { taskId?: string }[];
    polls: () => number;
  } {
    const sent: { taskId?: string }[] = [];
    let polls = 0;
    ...
    const waiting: Task = {
      id: 'task_wait',
      contextId: 'ctx',
      status: { state: TaskState.TASK_STATE_INPUT_REQUIRED, message: agentMessage('which region?'), timestamp: undefined },
      artifacts: [],
      history: [],
      metadata: undefined
    } as Task;
    const done: Task = {
      ...workingTask('task_wait'),
      status: {
        state: TaskState.TASK_STATE_COMPLETED,
        message: agentMessage('deployed to us-east'),
        timestamp: undefined
      }
    };
    return {
      client: {
        // A new task carries taskId '' (see userMessage); only the
        // continuation names the waiting task.
        sendMessage: async (params: { message: { taskId?: string } }) => {
          sent.push(params.message);
          return params.message.taskId ? done : waiting;
        },
        getTask: async () => {
          polls += 1;
          return waiting;
  ...
  it('waits for a human answer on input-required and continues the same task', async () => {
    vi.useFakeTimers();
    const cards = new CardStore(db);
    const jobs = new JobStore(db);
    const approvals = new ApprovalStore(db);
    const { client, sent, polls } = questioningClient();
    ...
    const gates = await approvals.list({ status: 'pending', scope: 'job' });
    expect(gates).toHaveLength(1);
    expect(gates[0]!.summary).toContain('which region?');

    await approvals.resolve(gates[0]!.approvalId, 'approve', { editedInput: { message: 'us-east' } });
    await vi.advanceTimersByTimeAsync(5_000);
    await run;

    // Answered, not re-created: the continuation names the waiting task.
    expect(sent).toHaveLength(2);
    expect(sent[1]?.taskId).toBe('task_wait');
    const text = events
      .filter(e => e.type === 'text')
      .map(e => (e as { text: string }).text)
      .join('');
    expect(text).toContain('deployed to us-east');
    // Answered on the first observation without polling a stuck task.
    expect(polls()).toBe(0);
  });

  it('fails with POLICY_DENIED when the human rejects the remote input', async () => {
    vi.useFakeTimers();
    const cards = new CardStore(db);
    const jobs = new JobStore(db);
    const approvals = new ApprovalStore(db);
    const { client, sent } = questioningClient();
    ...
    await vi.advanceTimersByTimeAsync(2_000);
    const gates = await approvals.list({ status: 'pending', scope: 'job' });
    await approvals.resolve(gates[0]!.approvalId, 'reject', { comment: 'too risky' });

    const pending = expect(run).rejects.toThrow(/rejected by a reviewer/);
    await vi.advanceTimersByTimeAsync(5_000);
    await pending;
    // Never answered the remote agent.
    expect(sent).toHaveLength(1);
  });

  it('files an unverified_card approval when the trust gate stops a job', async () => {
    const cards = new CardStore(db);
    const jobs = new JobStore(db);
    const approvals = new ApprovalStore(db);
    ...
    const gateway = new A2AGateway({
      ...
      clientProvider: async () => {
        throw new Error('must not be reached: trust fails first');
      }
    });

    // Unsigned fixture card, so verified-only refuses it at use time.
    const job = await remoteJob(cards, jobs, new AgentRegistry(db));
    ...
    await expect(drain()).rejects.toThrow(/unverified/);
    expect(await approvals.list({ status: 'pending', scope: 'unverified_card' })).toHaveLength(1);

    // A second blocked job files no second notice for the same card.
    await expect(drain()).rejects.toThrow(/unverified/);
    expect(await approvals.list({ status: 'pending', scope: 'unverified_card' })).toHaveLength(1);
  });
