...
describe('AgentRegistry.syncFromFiles', () => {
  ...
  it('creates file agents disabled, and never rewrites an explicit enable', async () => {
    const db = await migratedDb();
    const agents = new AgentRegistry(db);
    const dir = tempAgentsDir({ 'sec.md': '---\nname: sec\n---\nReview security.' });

    await agents.syncFromFiles(loadAgentFiles(dir));
    const synced = await agents.findByName('sec', '');
    expect(synced?.enabled).toBe(false);
    if (synced === undefined) throw new Error('expected the synced agent to exist');

    await agents.update(synced.id, { enabled: true });
    await agents.syncFromFiles(loadAgentFiles(dir));

    // Instructions still reconcile, but the operator's enable survives.
    expect((await agents.findByName('sec', ''))?.enabled).toBe(true);
    await db.close();
  });
