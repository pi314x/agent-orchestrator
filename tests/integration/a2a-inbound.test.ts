...
async function start(svc: Services, corsAllowedOrigins?: readonly string[]): Promise<A2AHttpHandle> {
...
describe('A2A inbound server', () => {
  ...
  // With default allow-all CORS the guard is bypassed by definition, so this
  // pins the restricted mode, where a stranger is still refused.
  it('refuses a request whose Origin header is not allowed, even with a valid Host', async () => {
    services = await testServices();
    server = await start(services, ['http://127.0.0.1']);
  ...
  it('admits any Origin by default, since CORS defaults to allow-all', async () => {
    services = await testServices();
    server = await start(services);
    const body = JSON.stringify({
      jsonrpc: '2.0',
    ...
    await expect(postWithOrigin(server.url, 'https://evil.example.com', body)).resolves.not.toBe(403);
  });
