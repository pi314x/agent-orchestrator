...
export interface TestServicesOptions {
  ...
  /** Compress the transient-retry backoff so a retry test needs no real waiting. */
  retry?: { maxRetries?: number; baseDelayMs?: number };
  /** Capture webhook delivery instead of hitting the network. */
  notifyFetch?: typeof fetch;
  /** Lock webhook callbacks to a host allow-list. */
...
export async function testServices(options: TestServicesOptions = {}): Promise<Services> {
  ...
  const services = createServices({
    ...
    ...(options.notifyFetch !== undefined && { notifyFetch: options.notifyFetch }),
    ...(options.webhookAllowedHosts !== undefined && { webhookAllowedHosts: options.webhookAllowedHosts })
  });
